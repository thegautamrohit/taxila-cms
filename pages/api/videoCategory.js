  ...
  switch (req.method) {
    case "GET":
      try {
        ...
        if (id) {
          sql = `
          SELECT * FROM video_category WHERE id = ${id}`;
        } else {
          sql = `
    ...
    case "POST":
      try {
        ...
        const sql = `
        INSERT into video_category (name) values (?)`;
    ...
    case "DELETE":
      try {
        const id = req.query.id;
        const valueParams = [];
        const sql = `DELETE FROM video_category WHERE id = ${id}`;
        const result = await query({ query: sql, value: valueParams });
    ...
    case "PATCH":
      try {
        const { title } = req.body;
        const id = req.query.id;
        let Item_Id = `SELECT * FROM video_category WHERE id = ${id}`;

        const findId = await query({ query: Item_Id });

        if (findId.length > 0) {
          const valueParams = [title];
          const sql = `UPDATE video_category SET name=?  WHERE id = ${id}`;
          const result = await query({ query: sql, value: valueParams });
