  ...
  switch (req.method) {
    ...
    case "POST":
      try {
        ...
        const sql = `
        INSERT into inspirationCategory (title) values (?)`;
    ...
    case "DELETE":
      try {
        const id = req.query.id;
        const valueParams = [];
        const sql = `DELETE FROM inspirationCategory WHERE id = ${id}`;
        const result = await query({ query: sql, value: valueParams });
    ...
    case "PATCH":
      try {
        const { title } = req.body;
        const id = req.query.id;
        let Item_Id = `SELECT * FROM inspirationCategory WHERE id = ${id}`;

        const findId = await query({ query: Item_Id });

        if (findId.length > 0) {
          const valueParams = [title];
          const sql = `UPDATE inspirationCategory SET title=?  WHERE id = ${id}`;
          const result = await query({ query: sql, value: valueParams });
