...
export default async function handler(req, res) {
  switch (req.method) {
    ...
    case "POST":
      try {
        const { date, category, link, images, website, edition , name } = req.body;

        const sql = `
        INSERT into media(date,category,link,images,website,edition, Name) values (?,?,?,?,?,?,?)`;

        const valueParams = [date, category, link, images, website, edition , name];
    ...
    case "PATCH":
      try {
        ...
        if (findId.length > 0) {
          const valueParams = [date, category, link, images, website, edition , name];
          const sql = `UPDATE media SET date=? ,category=? ,link=? ,images=? ,website=? ,edition=? ,Name=? WHERE id = ${id}`;
          const result = await query({ query: sql, value: valueParams });
